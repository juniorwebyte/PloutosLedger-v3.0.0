...
app.get('/api/cnpj/:cnpj', async (req, res) => {
  try {
    ...
    let data: any = await tryFetchJson(`https://brasilapi.com.br/api/cnpj/v1/${cnpj}`);
    ...
    const extractIE = (raw: any): string => {
      const direct =
      ...
      const arr = raw?.inscricoes_estaduais || raw?.inscricoesEstaduais || raw?.inscricoes || raw?.inscricoes_estaduais;
      if (Array.isArray(arr) && arr.length) {
        const pick =
          arr.find((x: any) => String(x?.ativo ?? x?.active ?? '').toLowerCase() === 'true') ||
          arr.find((x: any) => String(x?.situacao ?? x?.status ?? '').toLowerCase().includes('habil')) ||
          arr[0];
    ...
    const normalized = {
      ...
      qsa: (data.qsa || []).map((q: any) => ({ nome: q.nome_socio || q.nome || '', qual: q.qualificacao_socio || q.qual || '' })),
      status: 'OK',
    };
...
// ============================================================
// PDV ↔ Movimento de Caixa (Integração)
// ============================================================

type PDVSaleNormalized = {
  id: string;
...
function normalizeSale(input: any): PDVSaleNormalized | null {
  if (!input || typeof input !== 'object') return null;

  const id = String(input.id || input.sale_id || input.transaction_id || input.uuid || '').trim();
  const total = coerceNumber(input.total ?? input.net_total ?? input.amount ?? input.valor_total ?? input.valor ?? 0);
  const paymentMethod = String(input.paymentMethod ?? input.payment_method ?? input.method ?? input.metodo_pagamento ?? input.forma_pagamento ?? 'unknown');

  // Data/hora (aceita created_at/issue_date/date/time)
  const dateStr = String(input.date || input.data || '').trim();
  const timeStr = String(input.time || input.hora || '').trim();
  const createdAt = input.created_at || input.createdAt || input.issue_date || input.issued_at || input.timestamp || input.datetime;
  const dt = createdAt ? new Date(createdAt) : (dateStr ? new Date(dateStr + (timeStr ? `T${timeStr}` : 'T00:00:00')) : new Date());

  const date = /^\d{4}-\d{2}-\d{2}$/.test(dateStr) ? dateStr : toIsoDate(dt);
  const time = /^\d{2}:\d{2}(:\d{2})?$/.test(timeStr) ? (timeStr.length === 5 ? `${timeStr}:00` : timeStr) : toTime(dt);

  const itemsRaw = input.items || input.itens || input.products || [];
  const items = Array.isArray(itemsRaw)
    ? itemsRaw.map((it: any) => ({
        productId: String(it.productId ?? it.product_id ?? it.id ?? it.sku ?? ''),
        productName: String(it.productName ?? it.product_name ?? it.name ?? it.descricao ?? ''),
        quantity: coerceNumber(it.quantity ?? it.qtd ?? it.quantidade ?? 0),
        unitPrice: coerceNumber(it.unitPrice ?? it.unit_price ?? it.preco_unitario ?? it.price ?? 0),
        total: coerceNumber(it.total ?? it.total_price ?? it.valor_total ?? it.valor ?? 0),
      }))
    : undefined;

  const customerRaw = input.customer || input.cliente;
  const customer = customerRaw
    ? {
        name: String(customerRaw.name ?? customerRaw.nome ?? ''),
        document: customerRaw.document ?? customerRaw.cpf_cnpj ?? customerRaw.cpfcnpj ?? customerRaw.cpf ?? customerRaw.cnpj,
      }
    : undefined;

  const fiscalRaw = input.fiscal || input.fiscal_receipt || input.cupom || input.nfe;
  const fiscal = fiscalRaw
    ? {
        receiptNumber: fiscalRaw.receipt_number ?? fiscalRaw.numero ?? fiscalRaw.numero_cupom,
        series: fiscalRaw.series ?? fiscalRaw.serie,
        accessKey: fiscalRaw.access_key ?? fiscalRaw.chave_acesso,
        issuedAt: fiscalRaw.issue_date ?? fiscalRaw.data_emissao ?? fiscalRaw.issued_at,
        status: fiscalRaw.status,
      }
    : undefined;
  ...
  return {
    id: id || `sale_${Date.now()}_${Math.random().toString(36).slice(2)}`,
    date,
    time,
    total,
    paymentMethod,
...
// Sincroniza vendas do PDV (proxy server-side para evitar CORS e padronizar payload)
app.post('/api/pdv/sync', authMiddleware(false), async (req, res) => {
  try {
    const body = z.object({
    ...
    if (!body.success) return res.status(400).json({ error: 'payload inválido', details: body.error.flatten() });

    const { endpoint, apiKey, startDate, endDate } = body.data;

    // Anexar filtros em querystring sem assumir contrato do PDV
    ...
    try {
      ...
      if (!r.ok) return res.status(r.status).json({ error: 'falha ao consultar PDV', status: r.status, body: text.slice(0, 2000) });

      let json: any;
      try {
        json = JSON.parse(text);
      } catch {
        return res.status(502).json({ error: 'resposta do PDV não é JSON', body: text.slice(0, 2000) });
      }

      const list = Array.isArray(json) ? json : (Array.isArray(json?.sales) ? json.sales : (Array.isArray(json?.data) ? json.data : []));
      const normalized = (list as any[]).map(normalizeSale).filter(Boolean) as PDVSaleNormalized[];
    ...
    return res.status(500).json({ error: e?.message || 'erro ao sincronizar PDV' });
  }
});
