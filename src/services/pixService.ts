...
class PixService {
  ...
  async atualizarConfiguracao(config: Partial<CompanyConfig>): Promise<CompanyConfig> {
    ...
    // Notificar toda a aplicação (mesma aba) para sincronizar painéis imediatamente
    window.dispatchEvent(new CustomEvent('ploutos:company-config-updated', { detail: this.companyConfig }));
