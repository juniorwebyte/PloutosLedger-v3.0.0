...
class CashFlowBackupService {
  ...
  restoreBackup(backupId: string): BackupData | null {
    ...
    const currentData =
      localStorage.getItem(`cashFlowData:${new Date().toISOString().split('T')[0]}`) ||
    ...
    // Restaurar dados
    localStorage.setItem(`cashFlowData:${backup.date}`, JSON.stringify({
    ...
    // Compatibilidade: manter a chave legada apontando para o dia restaurado
