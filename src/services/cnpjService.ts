...
// Compatibilidade: alguns módulos do PDV usam campos em camelCase
export interface EmpresaData extends CNPJData {
  razaoSocial?: string;
  nomeFantasia?: string;
  dataAbertura?: string;
...
class CNPJService {
  ...
  async consultarCNPJ(cnpj: string): Promise<CNPJData | null> {
    try {
      ...
      // Preferir backend do próprio sistema (evita CORS e melhora estabilidade)
      try {
        ...
        try {
          ...
          if (response.ok) {
            ...
            const normalized: EmpresaData = {
              ...
              data_abertura: data.abertura || data.data_inicio_atividade || '',
              ...
              capitalSocial: data.capital_social ? Number(data.capital_social) : 0,
            };
            return normalized;
          }
        } finally {
        ...
        // Se o backend não estiver disponível, cai para as APIs públicas abaixo
      }
...
// Compatibilidade: alguns módulos importam como default
