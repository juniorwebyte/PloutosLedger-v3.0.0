...
import { LogOut, Calculator, TrendingUp, TrendingDown, Save, RotateCcw, Download, FileText, Building, Info, CheckCircle2, XCircle, AlertCircle, BarChart3, X, Filter, Bell, AlertTriangle, ExternalLink, Maximize2, Minimize2, Search, Keyboard, FileSpreadsheet, Clock, Lock, Unlock, CreditCard } from 'lucide-react';
...
function CashFlow({ isDemo = false, onBackToLanding }: CashFlowProps) {
  ...
  // Carregar/sincronizar configurações da empresa (para refletir mudanças do OwnerPanel em tempo real)
  useEffect(() => {
    const load = async () => setCompanyConfig(await pixService.obterConfiguracao());
    load();

    const onUpdated = () => load();
    const onStorage = (e: StorageEvent) => {
      if (e.key === 'company_config') load();
    };
    window.addEventListener('ploutos:company-config-updated', onUpdated as any);
    window.addEventListener('storage', onStorage);
    return () => {
      window.removeEventListener('ploutos:company-config-updated', onUpdated as any);
...
════════════════════════════════════════════════════════════════`;
  ...
  useEffect(() => {
    const savedData = localStorage.getItem(getCashFlowStorageKey()) || localStorage.getItem(getLegacyCashFlowStorageKey());
    if (savedData) {
      try {
