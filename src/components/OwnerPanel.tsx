...
export default function OwnerPanel({ isOpen, onClose, onConfigUpdate }: OwnerPanelProps) {
  ...
  const handleBuscarCNPJ = useCallback(async (cnpj: string) => {
    ...
    // Sem bloqueio por "CNPJ principal": cada cliente deve poder informar seu próprio CNPJ
    ...
    try {
      ...
      if (resultado) {
        ...
        setConfig(prev => ({
          ...
          cnpj: cnpjLimpo,
          inscricaoEstadual: resultado.inscricao_estadual || prev.inscricaoEstadual,
          endereco: {
            logradouro: resultado.logradouro || prev.endereco.logradouro,
  ...
  useEffect(() => {
    ...
    if (cepLimpo.length === 8 && !buscandoCEP && cepEncontrado === null) {
  ...
  useEffect(() => {
    ...
    if (cnpjLimpo.length === 14 && !buscandoCNPJ && cnpjEncontrado === null) {
      // Delay para não fazer muitas requisições enquanto digita
      const timer = setTimeout(() => {
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <div className="absolute right-3 top-1/2 -translate-y-1/2">
                      ...
                      {!buscandoCNPJ && cnpjEncontrado === null && config.cnpj.replace(/\D/g, '').length === 14 && (
                        <button
                          type="button"
