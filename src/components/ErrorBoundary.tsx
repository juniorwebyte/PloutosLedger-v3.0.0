...
export class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    ...
    // Salvar erro no localStorage para análise posterior (Sempre salvar para suporte técnico)
    try {
      const errorLogs = JSON.parse(localStorage.getItem('ploutos_error_logs') || '[]');
      errorLogs.push({
        error: error.message,
        stack: error.stack,
        componentStack: errorInfo.componentStack,
        timestamp: new Date().toISOString(),
        user: localStorage.getItem('caixa_user') || 'anonymous'
      });
      // Manter apenas os últimos 20 erros para não estourar o storage
      const recentLogs = errorLogs.slice(-20);
      localStorage.setItem('ploutos_error_logs', JSON.stringify(recentLogs));
    } catch (e) {
      console.error('Falha ao salvar logs de erro localmente');
    }
